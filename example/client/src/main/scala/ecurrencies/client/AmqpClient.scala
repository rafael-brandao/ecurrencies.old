...
sealed trait AmqpClient extends Actor with ActorLogging {
  ...
  protected def handleRequest(request: GeneratedMessage, ecurrencyId: String, serviceId: String)
...
class RpcClientProducer(val createChannel: () => Channel, val resultAggregator: ActorRef, exchangeName: String,
                        val sleepTime: Duration)(implicit timeout: Duration)
    ...
    try {
      ...
      delivery match {
        case Some(d) =>
          ...
          import d.{getBody => body, getEnvelope => envelope, getProperties => responseProperties}
